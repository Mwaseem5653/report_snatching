...
export default function ExcelAnalyzerClient() {
  ...
  const [eyeconTopN, setEyeconTopN] = useState(15);
              ...
              <div className="flex flex-col space-y-3 border p-3 rounded-lg bg-slate-50">
                ...
                {enableEyecon && (
                    <div className="pl-6 space-y-1 animate-in slide-in-from-top-1">
                        ...
                        type="number" 
                        value={eyeconTopN} 
                        onChange={(e) => setEyeconTopN(parseInt(e.target.value) || 15)}
                        min={1} 
                        max={50}
                        className="h-8 text-xs"
                        />
           ...
           <CardContent className="flex flex-col items-center justify-center h-full min-h-[300px] p-8 space-y-6 text-center">
              ...
              {!file ? (
                  <div className="relative group cursor-pointer w-full h-full flex flex-col items-center justify-center border-2 border-dashed border-blue-300 rounded-2xl hover:bg-blue-50 transition-colors py-10">
                      ...
                      <h3 className="text-lg font-semibold text-blue-900">Click to Upload Excel File</h3>
                      <p className="text-sm text-blue-600/70">Supports .xlsx and .csv formats</p>
                  </div>
              ) : (
                  <div className="w-full space-y-6">
                      <div className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl shadow-sm">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => { setFile(null); setResultUrl(null); }} className="text-red-500 hover:bg-red-50">Remove</Button>
                      </div>
                      ...
                      {resultUrl ? (
                          <div className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                              <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-xl flex items-center gap-3 text-emerald-800 text-sm font-medium">
                                  ...
                                  Processing Complete! Your report is ready.
                              </div>
                              <a 
                                href={resultUrl} 
                                download={`Analyzed_${file.name}`}
                              ...
                              >
                                <Download size={20} /> Download Analyzed Report
                              </a>
                              <Button variant="outline" onClick={() => { setFile(null); setResultUrl(null); }} className="w-full h-11 rounded-xl">Analyze Another File</Button>
                          </div>
                      ) : (
                          <>
                            <Button 
                            ...
                            >
                                {loading ? (
                                    ...
                                    <><Settings2 className="mr-2 h-5 w-5" /> Start Analysis</>
                                )}
                            </Button>
                            
                            {loading && (
                                <div className="p-4 bg-amber-50 text-amber-800 text-xs rounded-lg flex items-start gap-2 text-left">
                                    <AlertTriangle size={14} className="shrink-0 mt-0.5" />
                                    <p>Processing time depends on the "Top N" settings. Fetching API data may take a few moments.</p>
                                </div>
                            )}
                          </>
                      )}
