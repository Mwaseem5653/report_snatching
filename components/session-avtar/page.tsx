...
import { LogOut, User, ChevronDown, Bell, Coins, Menu, X, Clock, RefreshCcw } from "lucide-react";
        ...
        if (session?.authenticated) {
            fetch(getApiUrl("/api/auth/create-session"), { cache: "no-store" })
                ...
                .then(data => {
                    ...
                    else setSession(prev => ({ ...prev, ...data })); // Refresh live tokens/expiry
                })
                .catch(() => {});
        }
    }, 30000);
    ...
    <header className="w-full bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-200 sticky top-0 z-50">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 shrink-0">
          <button onClick={() => setMobileMenuOpen(true)} className="lg:hidden p-2 -ml-2 text-slate-600 hover:bg-slate-100 rounded-xl"><Menu size={24} /></button>
        ...
        <div className="flex items-center gap-2 md:gap-3 shrink-0" ref={dropdownRef}>
          <button onClick={() => setOpen((v) => !v)} className="flex items-center gap-2 pl-2 pr-1 py-1 rounded-full border border-slate-200 hover:bg-slate-50 transition-all relative">
          ...
          {open && (
            <div className="absolute right-4 top-full mt-2 w-64 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden animate-in fade-in slide-in-from-top-2 z-50">
              <div className="p-4 bg-slate-50 border-b border-slate-100">
                ...
                {/* Tokens logic based on strict roles */}
                {!["ps_user", "market_user"].includes(session?.role || "") && (
                    <div className="mt-3 flex flex-col gap-2">
                        ...
                        {/* Eyecon Tokens - Only for Super Admin or Eyecon Access */}
